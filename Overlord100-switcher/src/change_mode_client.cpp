...
public:
  ModeChangeClient() : Node("mode_change_client") {
    client_ = this->create_client<overlord100_interfaces::srv::ChangeMode>("change_mode");
  ...
  bool send_request(int mode) {
    auto request = std::make_shared<overlord100_interfaces::srv::ChangeMode::Request>();
    request->mode = mode;
...
int main(int argc, char **argv) {
  ...
  if (argc != 2) {
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Usage: change_mode_client <mode>");
    return 1;
  }
